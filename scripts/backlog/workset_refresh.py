...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh workset cache metadata for a backlog item.")
...
def main() -> int:
    ...
    if not ws_dir.exists():
        print(f"Workset not found: {ws_dir}")
        return 0
    import json
    meta = {}
    if meta_path.exists():
    ...
    message = f"Workset refreshed: {ws_dir.relative_to(repo_root).as_posix()}"
    with open(item.path, "a", encoding="utf-8") as f:
        f.write(f"\n{timestamp} [agent={args.agent}] {message}\n")

    print(f"Refreshed workset: {ws_dir}")
    return 0
