...
def main() -> int:
    ...
    enabled = bool(get_config_value(config, "index.enabled", False))

    refresh = args.refresh_index
    if refresh == "auto":
        refresh = "incremental" if enabled and backend == "sqlite" else "skip"

    python = sys.executable
    scripts_root = Path(__file__).resolve().parents[1]

    views_root = backlog_root / "views" / "_demo"
    items_root = backlog_root / "items"
    ensure_under_allowed(views_root, allowed_roots, "views-root")
    ...
    dashboards = [
        ("New,InProgress", "InProgress Work", "Active"),
        ("New", "New Work", "New"),
        ("Done", "Done Work", "Done"),
    ]

    # NoDBIndex (force file scan)
    for groups, title, suffix in dashboards:
        output = views_root / f"Dashboard_Demo_NoDBIndex_{suffix}.md"
        cmd = [
            python,
            str(generate),
            "--source",
    ...
    # DBIndex (force sqlite)
    for groups, title, suffix in dashboards:
        output = views_root / f"Dashboard_Demo_DBIndex_{suffix}.md"
        cmd = [
            python,
            str(generate),
            "--source",
            ...
            groups,
            "--title",
            f"{title} (Demo: DBIndex)",
        ]
        if args.config:
            cmd.extend(["--config", args.config])
        run_cmd(cmd, args.dry_run)

    # Tag view demos (versioning/release/milestones)
    tag_view = scripts_root / "backlog" / "generate_tag_view.py"
    tag_list = "versioning,release,milestone,0.0.1,0.0.2"

    nodb_tag_output = views_root / "Dashboard_Demo_NoDBIndex_Tags_Versioning.md"
    cmd = [
        python,
        str(tag_view),
        "--source",
        ...
        str(nodb_tag_output),
        "--title",
        "Versioning & Release (Demo: NoDBIndex)",
        "--tags",
        tag_list,
        "--agent",
        agent,
    ...
    db_tag_output = views_root / "Dashboard_Demo_DBIndex_Tags_Versioning.md"
    cmd = [
        python,
        str(tag_view),
        "--source",
        ...
        str(db_tag_output),
        "--title",
        "Versioning & Release (Demo: DBIndex)",
        "--tags",
        tag_list,
        "--agent",
        agent,
